...
def find_fvg(candles):
    """Look for a Fair Value Gap in the last 5 candles.
    Bullish FVG: candle[-3].high < candle[-1].low  (gap between them)
...
def find_order_block(candles, direction):
    ...
    For a bullish OB: last red (down) candle before a big up move.
    For a bearish OB: last green (up) candle before a big down move."""
    if len(candles) < 5:
        return None
    for i in range(len(candles) - 2, max(len(candles) - 10, 0), -1):
        ...
        # Strong next candle = at least 1.5x the current candle's body
...
def calc_trade(signal, entry, prev_high, prev_low, ob):
    if signal == "BUY":
        # SL below the order block if we have one, otherwise below prev low
